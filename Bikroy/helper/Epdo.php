...
class Epdo {
   ...
   /**
    ...
    /**
     * Call a select * from $param query that returns a row
     * @return first column of that row
     */
    public function getFromWhereVal($from_where) {
     ...
     public function callAfunction($func) {
        $stmt = $this->pdo->query('SELECT '.$func);
        $returns = $stmt->fetchAll(\PDO::FETCH_ASSOC);
        return $returns;
    }
}